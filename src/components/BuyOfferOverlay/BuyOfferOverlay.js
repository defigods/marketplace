...
const BuyOfferOverlay = (props) => {  
  ...
  const handleNext = () => {
    if ((activeStep + 1) === 1) {
      if (!props.userProvider.state.isLoggedIn) {
        ...
        //  TODO Remove timeout
        setTimeout(function () { sendBuyOffer(); }, 1500);
        ...
        return <div className="Overlay__body_cont">
          ...
          <div className="Overlay__lower">
            ...
            <div className="Overlay__buttons_container">
              <HexButton url="#" text="Place Order" className={`--purple ${bidValid ? '' : '--disabled'}`} onClick={handleNext}></HexButton>
  ...
  return <ReactCSSTransitionGroup
    ...
    <div key="bid-overlay-" to={props.url} className={`Overlay BuyOfferOverlay WhiteInputs ${props.className ? props.className : ''} --activeStep-${activeStep}`}>
