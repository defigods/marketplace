	...
	const mapFullInit = () => {
		if (document.getElementById('c-hex-full-map')) {
			...
			map.on('click', function (e) {
				...
				var geojson = geojson2h3.h3SetToFeatureCollection(Object.keys(newData), (hex) => ({ value: newData[hex] }));
				...
				$1('#c-fhi-name').innerHTML =
					form_h3_to_words(h3Geo)[0] + '.' + form_h3_to_words(h3Geo)[1] + '.' + form_h3_to_words(h3Geo)[2];
				if (!selected_source) {
			...
			reqwest({
				...
				success: function (resp) {
					...
					document
						...
						.insertAdjacentHTML('beforeend', resp['stats']['bountylands']['discovered']);
					document
						.getElementById('c-bounty-map__bounty-tokens')
						.insertAdjacentHTML('beforeend', resp['stats']['bountylands']['total_token']);
					...
					success: function (resp) {
						...
						map.addLayer({
							...
							paint: {
								...
								'circle-color': ['step', ['get', 'point_count'], '#006494', 100, '#44355D', 750, '#1a0731'],
			...
			reqwest({
				...
				success: function (resp) {
					...
					document
						...
						.insertAdjacentHTML('beforeend', resp['stats']['users']['registered']);
					document
