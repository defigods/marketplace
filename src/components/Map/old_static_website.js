...
const Application = (() => {
    ...
    if (document.getElementById('c-hex-full-map')) {
      ...
      map.on('load', () => {
        ...
        geocoder.on('result', function (ev) {
          ...
          const h3Geo = h3.geoToH3(ev.result.geometry.coordinates[0], ev.result.geometry.coordinates[1], 12)
          // document.getElementById('c-hex-map-info').innerHTML = h3Geo + ' = ' +  JSON.stringify(e.lngLat) 
          document.getElementById('c-hex-full-map-info').innerHTML = 'OVRLandID = ' + h3Geo
        });
      ...
      document.getElementById('c-hex-full-map-jump-to').addEventListener('click', function () {
        // Fly to a random location by offsetting the point -74.50, 40
        // by up to 5 degrees.
        map.flyTo({
          zoom: 18,
      ...
      map.on('mousemove', function (e) {
        const h3Geo = h3.geoToH3(e.lngLat['lat'], e.lngLat['lng'], 12)
        // document.getElementById('c-hex-map-info').innerHTML = h3Geo + ' = ' +  JSON.stringify(e.lngLat) 
        document.getElementById('c-hex-full-map-info').innerHTML = 'OVRLandID = ' + form_h3_to_words(h3Geo)[0] + "." + form_h3_to_words(h3Geo)[1] + "." + form_h3_to_words(h3Geo)[2]
      ...
      map.on('click', function (e) {
        ...
        console.log('OVRLandID = ' + form_h3_to_words(h3Geo)[0] + "." + form_h3_to_words(h3Geo)[1] + "." + form_h3_to_words(h3Geo)[2])
        ...
        $1('#c-fhi-coordi-cont').innerHTML = geojson['features'][0]['geometry']['coordinates'][0].join(' <br>')
        $1('#c-fhi-name').innerHTML = form_h3_to_words(h3Geo)[0] + "." + form_h3_to_words(h3Geo)[1] + "." + form_h3_to_words(h3Geo)[2]
        if (!selected_source) {
    ...
    if (document.getElementById('c-bounty-hex-map')) {
      ...
      reqwest({
        ...
        , success: function (resp) {
          ...
          document.getElementById("c-bounty-map__total-lands").insertAdjacentHTML('beforeend', resp['stats']['bountylands']['discovered'])
          document.getElementById("c-bounty-map__bounty-tokens").insertAdjacentHTML('beforeend', resp['stats']['bountylands']['total_token'])
          ...
          reqwest({
            ...
            , success: function (resp) {
              ...
              map.on('mousemove', disc_layerId, function (e) {
                ...
                population.innerHTML = '<strong>ID</strong>: ' + feature.properties.hex_id +
                  "<br><strong>Discovered</strong>: " + feature.properties.date +
                  "<br><strong  style='color:#009efa;' >User</strong>: <span>" + feature.properties.user_id + "</span>" +
                  "<br><strong>Prize index</strong>: " + feature.properties.prize_index +
                  "<br><strong>Prize Size</strong>: " + feature.properties.prize_value;

                overlay.appendChild(population);
                overlay.style.display = 'block';

                // Add features that share the same county name to the highlighted layer.
                map.setFilter('discovered-highlighted', ['in', 'user_id', feature.properties.user_id]);
    ...
    if (document.getElementById('c-user-stats')) {
      ...
      reqwest({
        ...
        , success: function (resp) {
          ...
          document.getElementById("c-bounty-map__total-lands").insertAdjacentHTML('beforeend', resp['stats']['bountylands']['discovered'])
          document.getElementById("c-bounty-map__bounty-tokens").insertAdjacentHTML('beforeend', resp['stats']['bountylands']['total_token'])
          document.getElementById("c-bounty-map__total-registered-users").insertAdjacentHTML('beforeend', resp['stats']['users']['registered'])
          document.getElementById("c-bounty-map__total-facebook-users").insertAdjacentHTML('beforeend', resp['stats']['users']['facebook'])
          document.getElementById("c-bounty-map__total-wechat-users").insertAdjacentHTML('beforeend', resp['stats']['users']['wechat'])
          document.getElementById("c-bounty-map__total-email-users").insertAdjacentHTML('beforeend', resp['stats']['users']['email'])
          document.getElementById("c-bounty-map__amazix-users").insertAdjacentHTML('beforeend', resp['stats']['users']['referral'])
          // #
      ...
      var data = {
        "api_key": "189bbbb00c5f1fb7fba9ad9285f193d1be46c0595051f6bb8c40b4e913461591"
      };
      var chart;
      reqwest({
        ...
        , success: function (resp) {
          ...
          document.getElementById("js-total-session").insertAdjacentHTML('beforeend', sum(session_counts))

          var registration_counts = resp.map(a => a.registered_users)
          document.getElementById("js-total-registration").insertAdjacentHTML('beforeend', sum(registration_counts))
            ...
            datePicker.addEventListener('change', function () {
              ...
              reqwest({
                ...
                , success: function (resp) {
                  ...
                  var dates = resp.map(a => a.date)
                  var session_counts = resp.map(a => a.active_users)
                  document.getElementById("js-total-session").innerHTML = sum(session_counts)

                  var registration_counts = resp.map(a => a.registered_users)
                  document.getElementById("js-total-registration").innerHTML = sum(registration_counts)
      ...
      reqwest({
        ...
        , success: function (resp) {
          each(resp['users'], function (index, user) {
            ...
            html_node += '</tr>'
            document.getElementById("c-bounty--user_table").insertAdjacentHTML('beforeend', html_node)

          })
          document.getElementById("c-bounty--user_total").insertAdjacentHTML('beforeend', resp['total'])
          document.getElementById("c-bounty--user_page_total").insertAdjacentHTML('beforeend', resp['number_pages'])
          document.getElementById("c-bounty--user_current-page").insertAdjacentHTML('beforeend', resp['current_page'])
        }
      })

      document.getElementById('c--bounty--user_prev').addEventListener('click', function (event) {
        event.preventDefault();
        var current_page = document.getElementById("c-bounty--user_current-page").innerText;
        if (current_page > 1) {
      ...
      document.getElementById('c--bounty--user_next').addEventListener('click', function (event) {
        event.preventDefault();
        var current_page = document.getElementById("c-bounty--user_current-page").innerText;
        var number_pages = parseInt(document.getElementById("c-bounty--user_page_total").innerText);
      ...
      reqwest({
        ...
        , success: function (resp) {
          each(resp['users'], function (index, user) {
            ...
            html_node += '</tr>'
            document.getElementById("c-bounty--user_referral_table").insertAdjacentHTML('beforeend', html_node)
          })
          document.getElementById("c-bounty--user_ref_total").insertAdjacentHTML('beforeend', resp['total'])
          document.getElementById("c-bounty--user_ref_page_total").insertAdjacentHTML('beforeend', resp['number_pages'])
          document.getElementById("c-bounty--user_ref_current-page").insertAdjacentHTML('beforeend', resp['current_page'])

        }
      })

      document.getElementById('c--bounty--user_ref_prev').addEventListener('click', function (event) {
        event.preventDefault();
        var current_page = document.getElementById("c-bounty--user_ref_current-page").innerText;
        if (current_page > 1) {
          document.getElementById("c-bounty--user_referral_table").innerHTML = "";
          document.getElementById("c-bounty--user_ref_current-page").innerHTML = "";
          current_page--;
          var data = {
            "api_key": "189bbbb00c5f1fb7fba9ad9285f193d1be46c0595051f6bb8c40b4e913461591",
            "page": current_page
      ...
      document.getElementById('c--bounty--user_ref_next').addEventListener('click', function (event) {
        event.preventDefault();
        var current_page = document.getElementById("c-bounty--user_ref_current-page").innerText;
        var number_pages = parseInt(document.getElementById("c-bounty--user_ref_page_total").innerText);
        if (current_page < number_pages) {
          document.getElementById("c-bounty--user_referral_table").innerHTML = "";
          document.getElementById("c-bounty--user_ref_current-page").innerHTML = "";
          current_page++;
  ...
  const languageManager = () => {
    ...
    button && (button.onmouseenter = function () {
      button.classList.add('show')
    });

    var button2 = document.querySelector("#submenu");
    button2 && (button2.onmouseleave = function () {
