...
const Map = (props) => {
	...
	function renderHighZoomMintedLands(hexagons) {
		...
		// Plot hexes
		const geojson = geojson2h3.h3SetToFeatureCollection(Object.keys(newData), (hex) => ({
			value: hexagons[hex]
		}));
	...
	function renderOwnedLandsCluster() {
		...
		map.addLayer({
			...
			paint: {
				"circle-color": ["step", ["get", "point_count"], "#514283", 100, "#514283", 750, "#331F56", ],
