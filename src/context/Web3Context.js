...
export class Web3Provider extends Component {
  ...
  initializeStore = async () => {
    ...
    this.context.actions.setUserBalance(
      parseFloat(ethers.utils.formatEther(reward).toString())
    )
    ...
    let CurveOVRSupply = TotalOVRSupply.sub(
      premine.mul(BigNumber.from(10 ** 9).mul(BigNumber.from(10 ** 9)))
    )

    let doublePremine = premine
      .mul(BigNumber.from(2))
      .mul(BigNumber.from(10 ** 9).mul(BigNumber.from(10 ** 9)))
    let vsBancorFormula = doublePremine.add(CurveOVRSupply)

    // Fallback bug
    if (CurveOVRSupply.isNegative()) {
      CurveOVRSupply = (
        parseFloat(ethers.utils.formatEther(reserve).toString()) / 0.07
      ).toFixed(2)
    } else {
      CurveOVRSupply = parseFloat(
        ethers.utils.formatEther(CurveOVRSupply).toString()
      ).toFixed(2)
    }
    ...
    // price of next Token
    let res =
      this.state.ibcoDAIReserve.toString() === '0'
        ? BigNumber.from(1)
  ...
  startHistoricLoop = async () => {
    // Loop through blocks
    let fromBlock = this.state.ibcoBlock.toNumber() - 50000
    let toBlock = this.state.ibcoBlock.toNumber() - 50000 + 9999
  ...
  calculateCustomSellSlippage = async (val) => {
    ...
    let pricePerToken =
      parseFloat(ethers.utils.formatEther(newPrice).toString()) / val
  ...
  calculateCustomBuySlippage = async (val) => {
    ...
    let pricePerToken =
      parseFloat(ethers.utils.formatEther(newPrice).toString()).toFixed(3) / val

    // console.log('pricePerToken STIMATO', pricePerToken)
    // console.log('pricePerToken CURRENT', (this.state.ibcoCurrentOvrPrice))
    let slippage =
      ((parseFloat(this.state.ibcoCurrentOvrPrice) - pricePerToken) /
        parseFloat(this.state.ibcoCurrentOvrPrice)) *
      100
    // console.log('slippage', slippage)
    return slippage
  }
  ...
  initializeContracts = async () => {
    ...
    let lightMintV2Signer = new ethers.Contract(
      config.apis.lightMintV2,
      lightMintV2ABI,
      this.state.signer
    )

    let lightMintV2Viewer = new ethers.Contract(
      config.apis.lightMintV2,
      lightMintV2ABI,
      this.state.provider
    )
    ...
    for (const _log of _logs) {
      ...
      try {
        ...
        switch (log.name) {
          case 'OpenBuyOrder': {
            if (
              this.state.address.toLowerCase() == log.args.buyer.toLowerCase()
            ) {
          ...
          case 'OpenSellOrder': {
            if (
              this.state.address.toLowerCase() == log.args.seller.toLowerCase()
            ) {
              log.transactionHash = transactionHash
              openSells.push(log)
            }
  ...
  removeOpenSellOrder(input) {
    var newIbcoOpenSellOrder = this.state.ibcoOpenSellOrders.filter((value) => {
      if (
        this.state.address.toLowerCase() === input[0].seller.toLowerCase() &&
        value.batchId === input[0].batchId
      ) {
        return value
      }
    })
  ...
  removeOpenBuyOrder(input) {
    var newIbcoOpenBuyOrder = this.state.ibcoOpenBuyOrders.filter((value) => {
      if (
        this.state.address.toLowerCase() === input[0].buyer.toLowerCase() &&
        value.batchId === input[0].batchId
      ) {
        return value
      }
